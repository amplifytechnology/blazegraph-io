...
use crate::rules::{RuleEngine, engine::DebugConfig};
use crate::preprocessors::{Preprocessor, TikaPreprocessor};
use crate::storage::{DocumentStorage, FileStorage, calculate_pdf_hash, calculate_config_hash};
use crate::types::*;
...
impl DocumentProcessor {
    ...
    #[cfg(feature = "jni-backend")]
    pub fn new_cli_jni_with_cache(jre_path: &std::path::Path, jar_path: &std::path::Path, cache_dir: &str) -> Result<Self> {
        let preprocessor = Box::new(TikaPreprocessor::new_with_jni(jre_path, jar_path)?);
    ...
    pub fn process_document_with_config_and_profiling(
        ...
        if enable_profiling {
            self.process_document_with_config_and_profiler(input_path, config, StepProfiler::new(true), skip_cache)
        } else if skip_cache {
            // Skip cache without profiling - use no-op profiler
            self.process_document_with_config_and_profiler(input_path, config, StepProfiler::new(false), skip_cache)
        } else {
    ...
    pub fn process_document_with_config(
        ...
        if let Some(cached) = self.storage.get_graph_output(&cache_key)? {
            ...
            println!("⏱️  Total processing time: {:.3}s (cached)", start_time.elapsed().as_secs_f64());
    ...
    fn process_document_with_config_and_profiler(
        ...
        let cached_result = if skip_cache {
            ...
            profiler.time_step("Cache Lookup", || {
        ...
        if let Some(cached) = cached_result {
            ...
            println!("⏱️  Total processing time: {:.0}ms (cached)", start_time.elapsed().as_millis());
            return Ok(cached.graph);
        }
        ...
        let graph = self.process_with_config_flow_and_profiler(input_path, config, &mut profiler)?;
        ...
        profiler.print_summary();
        println!("⏱️  Total processing time: {:.0}ms", start_time.elapsed().as_millis());
        Ok(graph)
    }
    ...
    fn process_with_config_flow(
        ...
        println!("⏱️  Preprocessing: {:.3}s", stage1_start.elapsed().as_secs_f64());
        ...
        println!("⏱️  Classification: {:.3}s", stage2_start.elapsed().as_secs_f64());
        ...
        println!("⏱️  Rule processing: {:.3}s", stage3_start.elapsed().as_secs_f64());

        let stage4_start = Instant::now();
        
        // Stage 4: Graph building (ParsedElements + Config → Graph)
        let graph = self.graph_builder.build_graph(parsed_elements)?;
        println!("⏱️  Graph construction: {:.3}s", stage4_start.elapsed().as_secs_f64());
    ...
    fn process_with_config_flow_and_profiler(
        ...
        let preprocessor_output = profiler.time_step("2. Markup → TextElements", || {
            self.preprocessor.parse_markup_to_preprocessor_output(&markup)
        })?;
        ...
        let parsed_elements = if config.minimal_parse {
            ...
            let font_size_analysis = profiler.time_step("4b. Font Analysis", || {
                self.rule_engine.analyze_font_sizes(&preprocessor_output.text_elements, &preprocessor_output.style_data)
            });
    ...
    pub fn process_document_with_options(
        ...
        println!("⏱️  Preprocessing complete: {:.3}s", start_time.elapsed().as_secs_f64());
        ...
        println!("⏱️  Text parsing: {:.3}s", step2_start.elapsed().as_secs_f64());
        ...
        println!("⏱️  Rule processing: {:.3}s", step3_start.elapsed().as_secs_f64());
        ...
        println!("⏱️  Graph construction: {:.3}s", step4_start.elapsed().as_secs_f64());
        println!("⏱️  Total processing time: {:.3}s", start_time.elapsed().as_secs_f64());
